   ...
   <div class="col-3 calendar-past-event-wrapper">
       ...
       <?php
           $args = array(
            ...
            $k = 0;
            $past_event_limit = 10;//get_field( 'past_events_amount', get_id_by_slug( 'calendar' ) );
            ...
            while($past_query->have_posts() ) : $past_query->the_post();
                ...
                else{// NOTE: we get all the upcoming events and add them to monthly variables
...
?>
       ...
       <div class="col-3 calendar-upcomming-container text-center">
           ...
           <button type="button"class="calendar-reset-btn btn btn-outline-dark" name="button" data-url="<?php echo admin_url('admin-ajax.php'); ?>">Reset</button>
           <h1 class="text-center text-uppercase"><?php the_field( 'get_involved_area_title', get_id_by_slug( 'calendar' ) ); ?></h1>
           <div class="calendar-btn-group btn-group-vertical w-100" role="group" aria-label="Basic example">
             <button onclick="window.location='<?php the_field('top_button_page_link', get_id_by_slug( 'calendar' ) ); ?>';" type="button" class="btn btn-lg btn-outline-dark"><?php the_field( 'top_button_text', get_id_by_slug( 'calendar') ); ?></button>
             <button onclick="window.location='<?php the_field('middle_button_page_link', get_id_by_slug( 'calendar' ) ); ?>';" type="button" class="btn btn-lg btn-outline-dark"><?php the_field( 'middle_button_text', get_id_by_slug( 'calendar') ); ?></button>
             <button onclick="window.location='<?php the_field('bottom_button_page_link', get_id_by_slug( 'calendar' ) ); ?>';" type="button" class="btn btn-lg btn-outline-dark"><?php the_field( 'bottom_button_text', get_id_by_slug( 'calendar') ); ?></button>
